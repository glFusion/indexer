...
*
* Reindex Functions (all AJAX based)
*
...
function INDEXER_getContentTypesAjax()
{
    global $_PLUGINS;
...
function INDEXER_removeOldContentAjax()
{
    global $_PLUGINS;
    ...
    Indexer\Indexer::Removeall($type);
...
function INDEXER_getContentListAjax()
{
    global $_INDEXER_CONF, $_PLUGINS;
...
function INDEXER_indexContentItemAjax()
{
    global $_INDEXER_CONF, $_PLUGINS;
    ...
    if ( is_array($contentInfo) && count($contentInfo) > 0 &&
        ...
        if (function_exists('plugin_commentsupport_'.$type ) || $type == 'article' ) {
            ...
            if ( $rc == true || $type == 'article') {
                indexer_IndexAll_comments($type, $id, $props['perms']);
            }
        }
...
function INDEXER_completeContentAjax()
{
    global $_PLUGINS;
